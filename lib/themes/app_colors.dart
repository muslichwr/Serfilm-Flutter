import 'package:flutter/material.dart';

class AppColors {
  static const background = Color(0xFF1A1A1A); // Hitam pekat
  static const surface = Color(0xFF2D2D2D); // Abu-abu gelap
  static const primary = Color(0xFF8AAEBD); // Biru cerah
  static const onPrimary = Colors.white;
  static const textPrimary = Color(0xFFE0E0E0);
  static const textSecondary = Color(0xFFA0A0A0);
  static const border = Color(0xFF444444);
  static const textFieldBg = Color(0xFF383838);
  static const accent = Color(0xFFC39BD3);
}
